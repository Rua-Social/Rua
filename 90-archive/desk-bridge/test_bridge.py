...
class GoogleMissTest(RuntimeCase):
    ...
    def test_pocket_brief_reads_todo_and_client_status(self):
        ...
        todo.write_text(
            f"# Todo\n\n## Open\n\n- {FRESH_TODO} Send the Northwind reply.\n"
        )
        ...
        self.assertIn("Do: Send the Northwind reply.", brief)
    ...
    def test_quoted_google_sentence_is_not_a_miss(self):
        (self.repo / "20-studio" / "todo.md").write_text(
            "# Todo\n\n## Open\n\n- 2026-08-18 Chase the Northwind deposit.\n"
        )
        with mock.patch.object(
            bridge,
            "run_grok",
    ...
    def test_voice_brainstorm_never_closes_the_list(self):
        ...
        self.assertIn("Chase the Northwind deposit.", todo.read_text())
        self.assertNotIn("Northwind", done.read_text())

    def test_google_miss_comes_back_from_grok_with_brief_and_park(self):
        (self.repo / "20-studio" / "todo.md").write_text(
            f"# Todo\n\n## Open\n\n- {FRESH_TODO} Chase the Northwind deposit.\n"
        )
        with mock.patch.object(
        ...
        self.assertIn("Do: Chase the Northwind deposit.", reply)
        self.assertIn("Do: Chase the Northwind deposit.", again)
        self.assertEqual(
    ...
    def test_pocket_brief_ranks_gated_do_over_softer_line(self):
        (self.repo / "20-studio" / "todo.md").write_text(
            ...
            f"- {FRESH_TODO} Chase the Northwind deposit on a dated thread.\n"
        )
        ...
        brief = bridge.pocket_brief(self.repo / "10-clients")
        self.assertIn("Do: Chase the Northwind deposit", brief)
    ...
    def test_pocket_brief_ranks_stale_gated_do_over_fresh_craft(self):
        ...
        (self.repo / "20-studio" / "todo.md").write_text(
            "# Todo\n\n## Open\n\n"
            f"- {FRESH_TODO} Do Acme craft edits\n"
            f"- {old} Chase the Northwind deposit on a dated thread.\n"
        )
        brief = bridge.pocket_brief(self.repo / "10-clients")
        self.assertIn("Do: STALE Chase the Northwind deposit", brief)
        self.assertNotIn("Acme", brief)
        self.assertNotIn("STALE: Chase the Northwind deposit", brief)
    ...
    def test_desk_ask_writes_todo_and_hides_trailers(self):
        ...
        with mock.patch.object(
            ...
            return_value=(
                ...
                "LIST+ Do | send the Contoso pack\n"
                "LIST+ Done | chase the Northwind deposit"
            ),
        ):
            reply = bridge.handle_prompt(
        ...
        self.assertIn("send the Contoso pack", open_text)
        self.assertNotIn("chase the Northwind deposit", open_text)
        self.assertIn("chase the Northwind deposit", done.read_text())
        self.assertNotIn("send the Contoso pack", self.lists.read_text())
    ...
    def test_list_write_does_not_duplicate_the_same_line(self):
        ...
        reply = bridge.persist_desk_writes(
            "Still that.\nLIST+ Do | chase the Northwind deposit"
        )
        self.assertEqual(reply, "Still that.\n\nAlready on the list.")
        self.assertEqual(
            todo.read_text().count("chase the Northwind deposit"), 1
        )
    ...
    def test_voice_ask_only_adds_an_explicit_do_line(self):
        ...
        with mock.patch.object(
            bridge,
            "run_grok",
            return_value="Noted.\nLIST+ Do | Send the Contoso nudge",
        ):
            reply = bridge.handle_prompt(
                "token", 420, 7,
                "Voice note: Add one todo: Send the Contoso nudge",
                from_voice=True,
            )
        self.assertEqual(reply, "Noted.")
        self.assertIn("Send the Contoso nudge", todo.read_text())
    ...
    def test_voice_ask_does_not_infer_a_do_line(self):
        ...
        with mock.patch.object(
            ...
            reply = bridge.handle_prompt(
                "token", 420, 7, "Voice note: nudge contoso", from_voice=True
            )
        self.assertIn("Voice did not add a todo", reply)
        self.assertNotIn("Send the Contoso nudge", todo.read_text())
...
class RulesTest(unittest.TestCase):
    ...
    def test_phone_rules_filter_intake_and_client_facing(self):
        ...
        self.assertIn("rua vault", rules)
        self.assertNotIn("10-clients/<slug>/", rules)
