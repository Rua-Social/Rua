...
| File | Ask for the file: "send me the month-1 PDF", or have a doc made and sent | The text answer, then the document. Repo, Downloads, or Desktop only. One file per ask. A raw `FILE+` line never shows. |
...
1. Asks for a file — "send me the month-1 PDF" — or asks for a doc to be made and sent.
