...
`todo-done.md`. Do not put todos here. Named-client notes live
in the vault.
...
- 17 Aug — Google live in this Grok window via the built-in connectors
  (Gmail & Calendar, Drive) from grok.com/connectors. That is the path. The local Google MCP
  servers in `~/.grok/config.toml` are not the path and stay unsigned. Gemini CLI is still API-key only
